...
def on_shape_selected(event):
    ...
    if shape == "Circle":
        ...
        # For square, ask for side length
...
def on_calculate_button_click():
    ...
    messagebox.showinfo("Result", f"The {calculation} of the {shape} is {result}")
...
shape_combobox = ttk.Combobox(root, values=["Circle", "Square"])  # Add more shapes if needed
...
calculation_combobox = ttk.Combobox(root, values=["Area", "Perimeter"])  # Add more calculations if needed
